...
# TensorRT with CUDA context isolation - attempting to re-enable
try:
    if TENSORRT_AVAILABLE:
        from .cuda_context_manager import get_context_manager
        context_manager = get_context_manager()
        
        if context_manager.health_status['tensorrt_healthy']:
            tensorrt_clip = create_tensorrt_clip(clip_model, data_processor, clip_device)
            print("TensorRT CLIP enabled with CUDA context isolation and stability fixes")
        else:
            tensorrt_clip = None
            print("TensorRT context unhealthy, using optimized PyTorch CLIP")
    else:
        tensorrt_clip = None
        print("TensorRT not available, using optimized PyTorch CLIP")
except Exception as e:
    tensorrt_clip = None
    print(f"TensorRT initialization failed: {e}, using optimized PyTorch CLIP")

# Set precision for better performance
torch.set_float32_matmul_precision('high')

def get_clip_status():
    """Get current CLIP processing status for logging"""
    if tensorrt_clip is not None:
        if hasattr(tensorrt_clip, 'use_tensorrt') and tensorrt_clip.use_tensorrt:
            return "TensorRT CLIP (context isolated)"
        else:
            return "PyTorch CLIP (TensorRT fallback)"
    else:
        return "PyTorch CLIP (TensorRT unavailable)"

print(f"CLIP Status: {get_clip_status()}")

def log_clip_health():
    """Log CLIP health status for monitoring"""
    if tensorrt_clip is not None and hasattr(tensorrt_clip, 'context_manager'):
        health = tensorrt_clip.context_manager.health_status
        logger.info(f"CUDA Context Health - TensorRT: {health.get('tensorrt_healthy', False)}, PyTorch: {health.get('pytorch_healthy', False)}")
        if health.get('last_error'):
            logger.warning(f"Last CUDA context error: {health.get('last_error')}")
...
def batch_classify_rois(regions_of_interest:list[ndarray]) -> list[BoundingBoxObject]:
    """Use CLIP to classify a batch of images (on GPU if available) with TensorRT optimization"""
    if tensorrt_clip is not None:
        # Use TensorRT CLIP with context isolation
        try:
            labels = [key.value for key in BoundingBoxObject]
            logits = tensorrt_clip.classify_images(regions_of_interest, labels, batch_size=32)
            ...
            logger.warning(f"TensorRT CLIP classification failed: {e}, falling back to PyTorch")
...
def evaluate_frame(
    ...
    check_clip_health_periodic(frame_id)
    ...
    if tensorrt_clip is not None:
        try:
            # Use TensorRT CLIP with context isolation
            step1_logits = tensorrt_clip.classify_images(rois, ["person", "grass"], batch_size=32)
            step1_probs = step1_logits.softmax(dim=1)
        except Exception as e:
            logger.warning(f"TensorRT CLIP step1 failed: {e}, falling back to PyTorch")
            # Fallback to PyTorch
    ...
    if ball_indexes:
        ...
        if tensorrt_clip is not None:
            try:
                # Use TensorRT CLIP with context isolation
                round_logits = tensorrt_clip.classify_images(football_rois, ["a photo of a round object on grass", "a random object"], batch_size=16)
                round_probs = round_logits.softmax(dim=1)
            except Exception as e:
                logger.warning(f"TensorRT CLIP round classification failed: {e}, falling back to PyTorch")
                # Fallback to PyTorch
        ...
        for j, i in enumerate(ball_indexes):
            ...
            if tensorrt_clip is not None:
                try:
                    # Use TensorRT CLIP with context isolation
                    step2_logits = tensorrt_clip.classify_images([football_rois[j]], [
                        "a small soccer ball on the field",
                        "just grass without any ball",
                        "other"
                    ], batch_size=1)
                    step2_probs = step2_logits.softmax(dim=1)[0]
                except Exception as e:
                    logger.warning(f"TensorRT CLIP step2 classification failed: {e}, falling back to PyTorch")
                    # Fallback to PyTorch
    ...
    if rois_for_person_refine:
        ...
        if tensorrt_clip is not None:
            try:
                # Use TensorRT CLIP with context isolation
                refine_logits = tensorrt_clip.classify_images(rois_for_person_refine, person_labels, batch_size=16)
                refine_probs = refine_logits.softmax(dim=1)
                refine_preds = refine_probs.argmax(dim=1)
            except Exception as e:
                logger.warning(f"TensorRT CLIP person classification failed: {e}, falling back to PyTorch")
                # Fallback to PyTorch
...
def check_clip_health_periodic(frame_id: int, check_interval: int = 100):
    """Periodically check and log CLIP health status"""
    if frame_id % check_interval == 0:
        log_clip_health()
        if tensorrt_clip is not None and hasattr(tensorrt_clip, 'consecutive_failures'):
            logger.info(f"TensorRT consecutive failures: {tensorrt_clip.consecutive_failures}")
...
def get_cached_text_features(labels_tuple):
    """Get cached text features or compute and cache them with health monitoring"""
    global _text_feature_cache, tensorrt_clip
    
    if labels_tuple in _text_feature_cache:
        return _text_feature_cache[labels_tuple]
    
    # Use TensorRT if available with health checks, otherwise fallback to PyTorch
    if tensorrt_clip is not None:
        try:
            text_features = tensorrt_clip.get_cached_text_features(labels_tuple)
        except Exception as e:
            logger.warning(f"TensorRT text encoding failed: {e}, falling back to PyTorch")
...
def check_and_recover_tensorrt():
    """Check TensorRT health and attempt recovery if needed"""
    global tensorrt_clip
    
    # If TensorRT is disabled, check if we can re-enable it
    if tensorrt_clip is None and TENSORRT_AVAILABLE:
        try:
            from .cuda_context_manager import get_context_manager
            context_manager = get_context_manager()
            
            # Check if context manager is healthy
            health_status = context_manager.check_health()
            
            if health_status['tensorrt_healthy']:
                # Attempt to recreate TensorRT CLIP
                logger.info("Attempting to re-enable TensorRT CLIP after recovery")
                tensorrt_clip = create_tensorrt_clip(clip_model, data_processor, clip_device)
                logger.info("TensorRT CLIP re-enabled successfully")
                return True
            else:
                logger.debug(f"TensorRT context still unhealthy: {health_status.get('last_error', 'Unknown error')}")
                return False
                
        except Exception as e:
            logger.warning(f"Failed to re-enable TensorRT: {e}")
            return False
    
    # If TensorRT is enabled, check its health
    elif tensorrt_clip is not None:
        try:
            # Simple health check - this will trigger internal health monitoring
            if hasattr(tensorrt_clip, '_check_health_and_recovery'):
                return tensorrt_clip._check_health_and_recovery()
            return True
        except Exception as e:
            logger.warning(f"TensorRT health check failed: {e}")
            tensorrt_clip = None
            return False
...
def batch_evaluate_frame_filter(frames: List[Dict], images: List[np.ndarray], batch_size: int = None, enable_class_limits: bool = False):
    ...
    if clip_indices:
        ...
        # Intelligent routing: TensorRT with health monitoring or PyTorch fallback
        use_tensorrt_for_batch = False
        
        # Check if we should attempt TensorRT with health monitoring
        if tensorrt_clip is not None or check_and_recover_tensorrt():
            try:
                # If we have TensorRT available, use it for this batch
                if tensorrt_clip is not None and hasattr(tensorrt_clip, 'classify_images'):
                    use_tensorrt_for_batch = True
                    print(f"[TensorRT] Processing {len(clip_rois)} ROIs with stabilized TensorRT CLIP")
                    
                    # Use TensorRT processing
                    logits = tensorrt_clip.classify_images(clip_rois, all_labels, clip_batch_size)
                    clip_predictions = logits.argmax(dim=1).cpu()
                    
            except Exception as e:
                logger.warning(f"TensorRT batch processing failed: {e}, falling back to PyTorch")
                use_tensorrt_for_batch = False
                tensorrt_clip = None  # Disable for this session
        
        # PyTorch fallback processing
        if not use_tensorrt_for_batch:
            clip_batch_size = 512
